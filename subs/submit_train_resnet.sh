...
for READ in ${CONTIGS[@]}; do
    for ORG in ${ORGS[@]}; do
        ...
        GV_DIR="${INPUT_DIR}/new_girus_genome_groups/${READ}"
            ...
            for NO_N in ${NO_NS[@]}; do
                for FLAG in "${FLAGS[@]}"; do
                    ...
                    if [[ $FLAG == *-g* ]]; then
                        ...
                        JOB_ID=`sbatch $ARGS --export=PY_ARGS --job-name=$(basename $OUTPUT_DIR) -e $STDERR_DIR/%x.err -o $STDOUT_DIR/%x.out ./runs/run_train_resnet.sh`
                    else
                        echo "Using CPU"
